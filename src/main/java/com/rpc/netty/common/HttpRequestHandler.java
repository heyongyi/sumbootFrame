...
public class HttpRequestHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected final HashMap<String,Object> clazztopath = jedisRead(appMap.get("moduleName")+"Cache"+"-mem","clazztopath");
    ...
    private void handleRequestUrl(String paramLine, HashMap<String, Object> urlParam) throws UnsupportedEncodingException {
        if (!StringUtils.isEmpty(paramLine)) {
    ...
    private void handleResponseCookies(HttpHeaders headers) {//响应中返回cookies
        for (Map.Entry<String, Object> entry : this.getCookies().entrySet()) {
            ...
            headers.add(HttpHeaderNames.SET_COOKIE, ServerCookieEncoder.encode(key,value));
        }
    }
    public HashMap<String, Object> jedisRead(String prefix, final String key) {
        ...
        JedisCluster jedis = new JedisClusterUtil().cluster(null,
        ...
        String json = jedis.hget(prefix,""+key);
        if (json != null) {
            try {
	...
	protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        ...
        /* +------------------------- 处理请求路径url参数 ----------------------------+ */
//      handleRequestUrl(StringUtils.split(request.uri(),"?")[1], urldata);
...
//        Class c = Class.forName("org.sumbootFrame.mvc.services.rpc."+executor+"Rpc"); //包名为interview
        String classPath = clazztopath.get(executor+"Rpc").toString();
